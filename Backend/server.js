...
mongoose
    ...
    app.use(
        morgan("method :url :status :response-time ms - :res[content-lenght]")
    );
...
app.get('/api/courses', async(req, res) => {
    try {
        ...
        logger.info(`Retrived ${courses.length} courses successfully`);
        res.json(courses);
    } catch (error) {
...
app.delete("/api/courses/:id", async(req, res) => {
    try {
        ...
        if (!course) {
            logger.warn("Course not found dor deletion:", {
                courseId: req.params.id,
            });
            return res.status(404).json({ message: "Course not found" });
        }
...
app.get("/api/students", async(req, res) => {
    try {
        ...
        logger.info(`Retrived ${students.length} students successfully`);
        res.json(students);
    } catch (error) {
        logger.error("Error fetching studnts:", error);
        res.status(500).json({ message: error.message });
    }
});
...
app.put("/api/students/:id", async(req, res) => {
    try {
        ...
        if (!student) {
            logger.warn("Student not found for update:", { studentId: req.params.id });
...
// SEARCH STUDENTS by name, course, email
...
// Dashboard Stats - calculate sucess rate
...
// Helper fuction for dashboard stats
